...
4. **Universal Successor Features and Fast Task Inference**
    ...
    - $$\pi$$가 reward 없이 학습되므로  그것을 maximize하기 위해 학습하는 것이 아니라 $$\pi'$$는 policy improvement에 의존합니다. 이를 여러 policies를 가지고 했을 때 좋은 성능이 나는 것을 기존에 연구가 되었는데, 이를 위해 policy-encoding mapping $$e : (\boldsymbol{S} →\boldsymbol{A})→\mathbb{R}^k$$ 를 사용합니다. 기존의 연구에서 universal successor feature(USFs)는 $$\boldsymbol{\psi}(s,a,e(\pi)) \equiv \boldsymbol{\psi}^\pi(s,a)$$로 정의되었는데, USFs를 사용하면, 모든 policy $$\pi$$에 대해 다음과 같이 간단하게 나타낼 수 있습니다.
        ...
        이 결과는 unsupervised pre-training을 통해 data-effciency를 늘릴 수 있습니다. 첫째로, unsupervised phase 동안, agent는 USFA $$\boldsymbol{\psi}_\theta$$를 배웁니다. 그렇게 되면, RL phase때 $$\boldsymbol{w}$$만을 근사하면 됩니다. 마지막으로 n개의 policies $$\pi_i$$와 policy $$\pi$$에 대해서 근사치가 적당히 맞다면, $$\pi$$가 improve될 것임을 보입니다.
