...
optimization가능합니다. 
q는 z를 predict하도록 학습됩니다. 
policy의 parameter $$\theta$$ 는 non-differentiable environment를 통해 학습해야 하기 때문에,
REINFORCE trick을 사용해 $$\log{q(z|s)}$$를 maximize하는 REINFORCE 수식과 동일하게 update할 수 있습니다.
...
SF에서는 주어진 task에 대해 reward 를 정의할 때 쓰이는 linear weight $$\bold{w}$$와
BMI의 objective에서는 task를 define하기 위한 z를 maximize하기 위한 reward $$\log{q(z|s)}$$를 같도록
제한하여 해결하는데, $$z \equiv \bold{w}$$가 되기 위해서는, $$r_{SF}(s; \bold{w}) = r_{BMI}(s;\bold{w})$$여야하는데,(이전 section에서의 reward와 같은형태) 이는 BMI discriminator q가 다음을 만족해야함을 의미합니다.
