...
- Training Objective
  ...
  - model을 학습시키기 위해서 다음과 같은 expression을 maximize합니다.(reward는 생략하지만 observation과 같이 joint probability로 등장해야함이 자명합니다.)

    $$\ln p(o_{1:T} \vert a_{1:T}) \triangleq \ln \mathbb{E}_{p(s_{1:T \vert a_{1:T}})}[\prod^T_{t=1} p(o_t\vert s_t)]$$

    left term의 likelihood는 right term과 같이 latent state에 대한 observation의 posterior로 정의할 수 있습니다. 그리고, elbo를 사용해 이 posterior를 optimize하자면 다음과 같습니다. 
